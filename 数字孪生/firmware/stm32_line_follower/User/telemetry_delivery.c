    ...
    uint8_t *overwrote)
{
    ...
    appended = telemetry_batch_append_timed(
        &delivery->pending, frame, frame_len, timing, &appended_overwrote);
    if (overwrote != 0) {
...
uint8_t telemetry_delivery_prepare(TelemetryDelivery *delivery)
{
    ...
    if (!telemetry_batch_copy_prefix(
            ...
            TELEMETRY_BATCH_DELIVERY_WINDOW_FRAMES)) {
        return 0U;
    }
                                           ...
                                           uint8_t *dropped)
{
    ...
    pending_count = telemetry_batch_count(&delivery->pending);
    if (pending_count > TELEMETRY_BATCH_DELIVERY_WINDOW_FRAMES) {
    ...
    return telemetry_delivery_prepare(delivery);
}
                                                   ...
                                                   uint32_t tick_ms)
{
    uint8_t count;
    if (delivery == 0) return;
    count = telemetry_batch_count(&delivery->pending);
    if (count != 0U) delivery->pending.timing[count - 1U].t_enqueue_ms = tick_ms;
}
                                               ...
                                               uint32_t tick_ms)
{
    ...
    for (i = 0U; i < inflight->count; i++) {
        delivery->inflight.timing[i].t_tx_start_ms = tick_ms;
    }
}
                                              ...
                                              uint32_t tick_ms)
{
    ...
    for (i = 0U; i < delivery->inflight.count; i++) {
        delivery->inflight.timing[i].t_send_ok_ms = tick_ms;
    }
}
