...
typedef void (*EspTxCoordinatorClearTelemetryFn)(void *ctx);
...
typedef struct {
    ...
    HealthStats  *health_stats;     /* terminal stats hook (health baseline) */
    ...
    void        *discard_ctx;
    EspTxCoordinatorClearTelemetryFn clear_telemetry;
    void        *clear_telemetry_ctx;
 ...
 *   health_stats  - pointer to the health_stats instance (health baseline,
 *                   Task 5 terminal hook); may be NULL in minimal tests
...
/* Install the full telemetry-batch clear operation.  The legacy
 * tele_pending pointer remains a compatibility fallback for small host
 * tests; firmware uses this callback to clear count and length together. */
void etc_set_telemetry_clear(EspTxCoordinator *c,
                             EspTxCoordinatorClearTelemetryFn clear_fn,
                             void *clear_ctx);

/* Return 1 when any higher-priority ACK/STATUS work is pending. */
uint8_t etc_telemetry_blocked(uint8_t retry_pending,
                               uint8_t pending_ack,
                               uint8_t pending_status);

/* Return 1 only when the current control/sensor phase is complete and no
 * higher-priority work is pending.  Droppable telemetry must stay behind the
 * phase boundary so a safety status discovered later in the same main-loop
 * iteration cannot be preceded by a newly-started telemetry transaction. */
 ...
 *   1. Settle any in-flight CipsendTx tag via hstats_tx_boundary_abort
 *      (P0-3: health→health_failed++, telemetry→telemetry_tx_failed++).
 *   2. Abort the old CipsendTx (force-reset to IDLE).
 *   3. Drop all retained ACK/STATUS via txfq_check_generation().
 *   4. Clear the telemetry latest-wins slot.
 *   5. Discard all pending bytes from the TX ring via the callback.
 *
 * now_ms is the monotonic tick at the boundary; it is used only for the
 * honest duration of an aborted in-flight health transaction (never guessed).
 *
 ...
 * now_ms is used for the in-flight tag settlement (see etc_check_boundary).
 *
 ...
 *   0. Before handling, invoke hstats_tx_terminal(health_stats, tag, result,
 *      timeout_aborted, entered_send_data, now_ms) where entered_send_data is
 *      derived from tx->state ∈ {SEND_DATA, WAIT_SENDOK} (design §4.3).
