...
static int test_pending_status_query_tracks_fifo_without_consuming(void)
{
    ...
    twin_control_init(&k_baseline);
    CHECK(twin_control_has_pending_status() == 1U);
    CHECK(twin_control_consume_pending_status(&status) == 1U);
    CHECK(twin_control_has_pending_status() == 0U);
    ...
    CHECK(twin_control_report_line_lost(TWIN_CONTROL_LINE_LOST_MAX_MS) == 1U);
    CHECK(twin_control_has_pending_status() == 1U);
    CHECK(twin_control_consume_pending_status(&status) == 1U);
    CHECK(strcmp(status.reason, "LINE_LOST") == 0);
    CHECK(twin_control_has_pending_status() == 0U);
    (void)active;
    return 0;
}
...
/* [HB-1] Valid H frame with matching campaign/run sets pending + counts */
static int test_heartbeat_valid_increments_count(void)
{
    TwinControlStatus st;
    TwinControlResult result;
    char frame[96];
    ...
    /* Establish identity + clear inhibit via START. */
    make_frame(frame, sizeof(frame), "R,soak,soak0000002,START");
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_motion_inhibited() == 0U);
    twin_control_consume_pending_status(&st);   /* drain RUNNING */

    CHECK(twin_control_consume_heartbeat_pending() == 0U);  /* nothing yet */

    /* P1-4: count increments AT PARSE for each validated/matched H (even
       if pending was already 1); twin_control_heartbeat on consume only
       updates last_seen/lease, never re-counts. */
    make_frame(frame, sizeof(frame), "H,soak,soak0000002");
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_heartbeat_count() == 1U);   /* counted at parse */
    CHECK(twin_control_consume_heartbeat_pending() == 1U);
    CHECK(twin_control_consume_heartbeat_pending() == 0U);  /* consumed */

    twin_control_heartbeat(1000U);
    CHECK(twin_control_heartbeat_count() == 1U);   /* not double-counted */
    return 0;
}

/* [HB-2] Bad checksum / campaign mismatch / run mismatch -> no set, no count */
static int test_heartbeat_invalid_or_mismatch_ignored(void)
{
    TwinControlStatus st;
    TwinControlResult result;
    char frame[96];
    ...
    frame[strlen(frame) - 3U] = '0';
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_consume_heartbeat_pending() == 0U);

    /* Campaign mismatch. */
    make_frame(frame, sizeof(frame), "H,other,soak0000002");
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_consume_heartbeat_pending() == 0U);

    /* Run mismatch. */
    make_frame(frame, sizeof(frame), "H,soak,otherrun");
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_consume_heartbeat_pending() == 0U);

    /* None of the rejected H frames counted (pending never set, so the
       main loop never calls twin_control_heartbeat). */
    CHECK(twin_control_heartbeat_count() == 0U);
    return 0;
}

/* [HB-3] Lease opens on START edge; 999ms no timeout, 1000ms timeout */
static int test_heartbeat_lease_timeout(void)
{
    TwinControlStatus st;
    TwinControlResult result;
    char frame[96];
    ...
    /* First tick after START opens the lease at now=5000. */
    CHECK(twin_control_heartbeat_tick(5000U) == 0U);
    CHECK(twin_control_lease_active(5500U) == 1U);   /* gap 500 < 1000 */
    CHECK(twin_control_heartbeat_tick(5999U) == 0U); /* gap 999 < 1000 */
    CHECK(twin_control_lease_active(5999U) == 1U);

    /* now+1000 -> timeout. */
    CHECK(twin_control_heartbeat_tick(6000U) == 1U);
    CHECK(twin_control_heartbeat_timeout_count() == 1U);
    CHECK(twin_control_motion_inhibited() == 1U);
    CHECK(twin_control_heartbeat_last_reason() == HEALTH_HEARTBEAT_REASON_HEARTBEAT);

    /* S frame reason remains TIMEOUT (compat). */
    CHECK(twin_control_consume_pending_status(&st) == 1U);
    CHECK(strcmp(st.state, "STOPPED") == 0);
    CHECK(strcmp(st.reason, "TIMEOUT") == 0);
    return 0;
...
/* [HB-4] Initial inhibited state never opens a lease or times out */
static int test_heartbeat_tick_initial_inhibited(void)
{
    TwinControlStatus st;
    twin_control_init(&k_baseline);
    twin_control_consume_pending_status(&st);
    CHECK(twin_control_motion_inhibited() == 1U);
    CHECK(twin_control_heartbeat_tick(100U) == 0U);
    CHECK(twin_control_heartbeat_tick(5000U) == 0U);
    CHECK(twin_control_heartbeat_timeout_count() == 0U);
    return 0;
}

/* [HB-5] STOPPED H only counts; re-START re-opens lease via edge */
static int test_heartbeat_stopped_only_counts(void)
{
    TwinControlStatus st;
    TwinControlResult result;
    char frame[96];
    ...
    CHECK(feed(frame, &result) == 0U);
    twin_control_consume_pending_status(&st);
    CHECK(twin_control_heartbeat_tick(1000U) == 0U);  /* open lease */

    /* STOP -> inhibited, g_was_running=0. */
    make_frame(frame, sizeof(frame), "R,soak,soak0000002,STOP");
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_motion_inhibited() == 1U);
    twin_control_consume_pending_status(&st);

    /* H arrives while STOPPED: counted at parse, last_seen updated on
       consume, but lease NOT renewed (STOPPED never renews). */
    make_frame(frame, sizeof(frame), "H,soak,soak0000002");
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_heartbeat_count() == 1U);   /* counted at parse */
    CHECK(twin_control_consume_heartbeat_pending() == 1U);
    twin_control_heartbeat(2000U);
    CHECK(twin_control_heartbeat_count() == 1U);
    CHECK(twin_control_lease_active(2000U) == 0U);            /* no lease */
    CHECK(twin_control_heartbeat_age_ms(2500U) == 500U);      /* from last_seen */

    /* Restore + re-START re-opens the lease fresh. */
    CHECK(twin_control_apply_pending(&active, &k_baseline, &result) == 1U);
    make_frame(frame, sizeof(frame), "R,soak,soak0000002,START");
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_motion_inhibited() == 0U);
    ...
    CHECK(twin_control_heartbeat_tick(3000U) == 0U);  /* re-open lease */
    CHECK(twin_control_heartbeat_tick(3999U) == 0U);
    CHECK(twin_control_heartbeat_tick(4000U) == 1U);  /* fresh 1s lease expired */
    CHECK(twin_control_heartbeat_timeout_count() == 1U);
    return 0;
}

/* [HB-6] heartbeat_age_ms sentinel + now-last */
static int test_heartbeat_age(void)
{
    TwinControlStatus st;
    TwinControlResult result;
    char frame[96];
    ...
    twin_control_init(&k_baseline);
    twin_control_consume_pending_status(&st);
    /* Never received any valid heartbeat -> sentinel. */
    CHECK(twin_control_heartbeat_age_ms(500U) == 0xFFFFFFFFU);
    ...
    CHECK(twin_control_consume_heartbeat_pending() == 1U);
    twin_control_heartbeat(1000U);
    CHECK(twin_control_heartbeat_age_ms(1500U) == 500U);
    return 0;
}

/* [HB-6b] P1-4: two valid H in the same RX drain count separately at parse,
   but pending stays a single boolean and one consume renews the lease once. */
static int test_heartbeat_multiple_same_drain(void)
{
    TwinControlStatus st;
    TwinControlResult result;
    char frame[96];
    ...
    /* Both counted BEFORE any consume. */
    CHECK(twin_control_heartbeat_count() == 2U);
    /* pending is still a single boolean. */
    CHECK(twin_control_consume_heartbeat_pending() == 1U);
    CHECK(twin_control_consume_heartbeat_pending() == 0U);

    /* One consume → last_seen + lease renewed once; count unchanged. */
    twin_control_heartbeat(1000U);
    CHECK(twin_control_heartbeat_count() == 2U);
    CHECK(twin_control_heartbeat_age_ms(1200U) == 200U);
    CHECK(twin_control_lease_active(1500U) == 1U);
    return 0;
}

/* [HB-7] motion_state mapping INIT/RUNNING/STOPPED/TIMEOUT/LINE_LOST */
static int test_health_motion_state_mapping(void)
{
    TwinControlStatus st;
    TwinControlResult result;
    char frame[96];
    ...
    twin_control_init(&k_baseline);
    twin_control_consume_pending_status(&st);
    CHECK(twin_control_health_motion_state() == HEALTH_MOTION_STATE_INIT);

    make_frame(frame, sizeof(frame), "R,soak,soak0000002,START");
    CHECK(feed(frame, &result) == 0U);
    twin_control_consume_pending_status(&st);
    CHECK(twin_control_health_motion_state() == HEALTH_MOTION_STATE_RUNNING);

    make_frame(frame, sizeof(frame), "R,soak,soak0000002,STOP");
    CHECK(feed(frame, &result) == 0U);
    CHECK(twin_control_health_motion_state() == HEALTH_MOTION_STATE_STOPPED);

    twin_control_init(&k_baseline);
    twin_control_consume_pending_status(&st);
    twin_control_timeout();
    CHECK(twin_control_health_motion_state() == HEALTH_MOTION_STATE_TIMEOUT);
    ...
    CHECK(twin_control_report_line_lost(0U) == 0U);
    CHECK(twin_control_report_line_lost(1000U) == 1U);
    CHECK(twin_control_health_motion_state() == HEALTH_MOTION_STATE_LINE_LOST);
    return 0;
}

/* [HB-8] legacy timeout records LEGACY_TIMEOUT reason */
static int test_heartbeat_legacy_timeout_reason(void)
{
    TwinControlStatus st;
    twin_control_init(&k_baseline);
    twin_control_consume_pending_status(&st);
    twin_control_timeout();
    CHECK(twin_control_heartbeat_last_reason() == HEALTH_HEARTBEAT_REASON_LEGACY_TIMEOUT);
    return 0;
}

/* ===== End Health baseline heartbeat tests ===== */
...
static int run_all_tests(void)
{
    ...
    if (test_pending_status_query_tracks_fifo_without_consuming()) return 1;
    ...
    if (test_heartbeat_valid_increments_count()) return 1;
    if (test_heartbeat_invalid_or_mismatch_ignored()) return 1;
    if (test_heartbeat_lease_timeout()) return 1;
    if (test_heartbeat_tick_initial_inhibited()) return 1;
    if (test_heartbeat_stopped_only_counts()) return 1;
    if (test_heartbeat_age()) return 1;
    if (test_heartbeat_multiple_same_drain()) return 1;
    if (test_health_motion_state_mapping()) return 1;
    if (test_heartbeat_legacy_timeout_reason()) return 1;
    /* --- end Health baseline heartbeat --- */
