...
# ============================================================
# 无序点遍历 / 轮廓伪装中心线 拒绝测试 (Task 4B-2 Reacceptance)
# ============================================================


def test_centerline_ordered_monotonic_on_straight_track():
    """RED→GREEN: 直线赛道的中心线应单调有序，不出现回溯。

    提取 3px 宽竖线的中轴中心线，验证 y 坐标严格单调递增，
    即点列沿赛道方向前进，没有来回跳跃（无序遍历）的情况。
    """
    from v1_twin.v1_twin_track_map import extract_medial_centerline

    mask = np.zeros((60, 60), dtype=np.uint8)
    mask[:, 28:32] = 1  # 4px 宽竖线

    centerline = extract_medial_centerline(mask)
    assert len(centerline) >= 10, f"应有足够中心线点，实际 {len(centerline)}"

    # 提取 y 坐标序列：应单调（沿赛道方向前进）
    ...
    total = len(diffs)
    # 至少 90% 的步进方向一致（允许少量振荡）
    monotonic_ratio = dominant / total if total > 0 else 0.0
    assert monotonic_ratio >= 0.90, (
        f"中心线未沿赛道单调前进: {pos}/{total} 正向, {neg}/{total} 反向, "
        f"单调比={monotonic_ratio:.2f}"
    )


def test_centerline_ordered_no_large_backtrack():
    """RED→GREEN: 中心线不应出现大幅回溯（跨越空白区域后跳回）。

    使用 U 形弯道掩码，验证累计路径长不超过欧氏跨度 × 合理倍数。
    如果点列在空白区域跳跃，累计路径会异常长。
    """
    from v1_twin.v1_twin_track_map import extract_medial_centerline

    # 构造 U 形赛道：左竖线 + 底部弯 + 右竖线
    size = 80
    ...
    centerline = extract_medial_centerline(mask)
    assert len(centerline) >= 20, f"U 形赛道应有足够中心线点，实际 {len(centerline)}"

    # 累计路径长 vs 端点欧氏距离
    total_path = 0.0
    for i in range(len(centerline) - 1):
        total_path += math.dist(centerline[i], centerline[i + 1])

    # 端点间欧氏距离（起点 vs 终点）
    if len(centerline) >= 2:
        euclidean_span = math.dist(centerline[0], centerline[-1])
        # U 形路径长度应显著大于端点直线距离（证明走了弯道）
        # 但不应超过直线距离的 20 倍（排除严重无序跳跃）
        if euclidean_span > 1.0:
            ratio = total_path / euclidean_span
            assert 1.5 <= ratio <= 20.0, (
                f"路径/跨度比异常: {ratio:.1f}（路径={total_path:.0f}px, 跨度={euclidean_span:.0f}px）"
            )


def test_medial_centerline_not_contour():
    """RED→GREEN: 中轴中心线不应只是轮廓。

    对 10px 宽轨道，中心线上任一点的边界距离应 ≥ 2px（不在边界上），
    证明确实是"中轴"而非"轮廓边缘"。
    """
    from v1_twin.v1_twin_track_map import (
        compute_center_to_boundary_distance,
        extract_medial_centerline,
    )

    mask = np.zeros((80, 80), dtype=np.uint8)
    mask[:, 35:45] = 1  # 10px 宽竖线

    centerline = extract_medial_centerline(mask)
    assert len(centerline) > 0, "应有中心线点"

    dists = compute_center_to_boundary_distance(centerline, mask)
    nonzero = [d for d in dists if d > 0]
    assert len(nonzero) > 0, "应有非零距离点"

    # 中轴点到边界距离应接近线宽/2 = 5px，容差内至少 ≥ 2px
    min_dist = min(nonzero)
    median_dist = sorted(nonzero)[len(nonzero) // 2]
    assert min_dist >= 2.0, (
        f"中心线贴边（轮廓伪装）: min boundary dist={min_dist:.1f}px, "
        f"median={median_dist:.1f}px"
    )
    # 中位数应接近线宽的一半
    assert 3.0 <= median_dist <= 7.0, (
        f"中轴距离异常: median={median_dist:.1f}px, 预期 ≈5.0px"
    )


# ============================================================
# Waypoint-constrained 开放路线提取 (Task 4B-2 C1 open route)
# ============================================================


def _make_loop_with_start_and_finish_spurs(size=100):
    """合成带发车支线与终点支线的环形赛道掩码。

    - 矩形环: x∈[20,80], y∈[25,75]，壁厚 10px
    - 发车支线: 底部 (x≈52) 向下伸至 y≈95
    - 终点支线: 底部 (x≈40) 向下伸至 y≈86

    7 个测试锚点全部位于该掩码骨架 8px 内。
    """
    m = np.zeros((size, size), dtype=np.uint8)
    m[23:77, 18:28] = 1   # 左壁
    m[23:77, 72:82] = 1   # 右壁
    m[23:33, 18:82] = 1   # 顶壁
    m[67:77, 18:82] = 1   # 底壁
    m[77:95, 48:56] = 1   # 发车支线（中心 x≈52）
    m[77:86, 36:44] = 1   # 终点支线（中心 x≈40）
    return m
...
def test_waypoint_route_chooses_requested_side_of_loop():
    """RED: waypoint 约束提取应按请求侧绕环，锚点顺序出现。"""
    ...
    assert len(points) > 0
    waypoint_indices = _nearest_point_indices(
        points,
        ((50, 95), (50, 75), (80, 75), (80, 25), (20, 25), (20, 75), (45, 75)),
    )
...
def test_waypoint_route_rejects_anchor_outside_snap_radius():
    """RED: 超出吸附半径的锚点必须抛 ValueError。"""
    mask = _make_loop_with_start_and_finish_spurs()
    with pytest.raises(ValueError, match="cannot snap waypoint"):
        extract_waypoint_constrained_centerline(
            mask, ((50, 95), (0, 0)), snap_radius_px=3.0
        )


def test_waypoint_route_has_no_repeated_undirected_edge():
    """RED: 开放路线不得重复使用任何无向边（无折返/回绕）。"""
    mask = _make_loop_with_start_and_finish_spurs()
    points = extract_waypoint_constrained_centerline(
        mask, ROUTE_WAYPOINTS, snap_radius_px=8.0
    )
...
def _make_cropped_bottom_track_mask(size=100):
    """L 形轨道，底臂紧贴图像底边 y=size-1（模拟俯视图中赛道被底边裁切）。

    - 竖臂 x∈[48,52), y∈[25,size)（4px 宽）
    - 底臂 y=size-1（1px 高，骨架恰落在图像底边上）
    旧门禁（jump/blank/edge）对该掩码全绿，但掩码与中心线都接触图像边界。
    """
    m = np.zeros((size, size), dtype=np.uint8)
    m[25:size, 48:52] = 1
    m[size - 1:size, 30:60] = 1
    return m


def _make_clean_margined_track_mask(size=100):
    """L 形轨道，整体位于画面内部（四周留白 >20px），不接触任何边界。"""
    m = np.zeros((size, size), dtype=np.uint8)
    m[25:80, 48:52] = 1
    m[76:80, 30:52] = 1
    return m
...
def test_border_gate_catches_crop_that_old_gates_miss():
    """RED: 旧门禁（跳变/空白/重复边）对裁切掩码全绿，新边界指标必须拦截。

    证明独立审核缺口：被画面底边裁切的轨道仍能干净提取，旧 gates 一律通过；
    新增 `compute_border_coverage_metrics` 必须把这种输入判定为 input_cropped。
    """
    from v1_twin.v1_twin_track_map import compute_border_coverage_metrics

    mask = _make_cropped_bottom_track_mask()
    route = extract_waypoint_constrained_centerline(
        mask, ((50, 30), (50, 99), (60, 99)), snap_radius_px=6.0, min_spur_length=2
    )
    assert len(route) > 0

    # 旧门禁在裁切输入上全部通过 → 旧逻辑无法识别裁切
    max_jump = max(math.dist(a, b) for a, b in zip(route, route[1:]))
    blank = [p for p in route if mask[int(round(p[1])), int(round(p[0]))] != 1]
    edges = [tuple(sorted((a, b))) for a, b in zip(route, route[1:])]
    assert max_jump <= 2.0
    assert len(blank) == 0
    assert len(edges) == len(set(edges))

    # 新边界指标必须识别裁切
    m = compute_border_coverage_metrics(mask, route)
    assert m.input_cropped is True
    assert m.mask_border_touch_count > 0
    assert m.route_border_point_count > 0
    assert m.route_points_within_2px_of_border > 0
    assert m.min_route_border_clearance_px == 0.0


def test_border_metrics_clean_mask_not_cropped():
    """GREEN: 四周留白的干净掩码不得被误判为裁切。"""
    from v1_twin.v1_twin_track_map import compute_border_coverage_metrics

    mask = _make_clean_margined_track_mask()
    route = extract_waypoint_constrained_centerline(
        mask, ((50, 30), (50, 76), (35, 77)), snap_radius_px=6.0, min_spur_length=2
    )
    assert len(route) > 0
    m = compute_border_coverage_metrics(mask, route)
    assert m.input_cropped is False
    assert m.mask_border_touch_count == 0
    assert m.route_border_point_count == 0
    assert m.route_points_within_2px_of_border == 0
    assert m.min_route_border_clearance_px >= 10.0


def test_border_metrics_exact_counts_cropped():
    """GREEN: 精确计数——掩码触点与路线边界点逐一核对。"""
    from v1_twin.v1_twin_track_map import compute_border_coverage_metrics

    mask = np.zeros((20, 30), dtype=np.uint8)
    mask[0, 5:10] = 1   # 顶边 5 触点
    mask[19, 20] = 1    # 底边 1 触点
    mask[10, 15] = 1    # 内部（不触边）
    route = ((7.0, 0.0), (20.0, 19.0), (15.0, 10.0))
    m = compute_border_coverage_metrics(mask, route)
    assert m.mask_border_touch_count == 6
    assert m.route_border_point_count == 2
    assert m.route_points_within_2px_of_border == 2
    assert m.min_route_border_clearance_px == 0.0
    assert m.input_cropped is True


def test_border_metrics_exact_counts_clean():
    """GREEN: 内部掩码精确计数全零，且最小余量 ≥5px。"""
    from v1_twin.v1_twin_track_map import compute_border_coverage_metrics

    mask = np.zeros((20, 30), dtype=np.uint8)
    mask[5:15, 10:20] = 1
    route = ((12.0, 8.0), (15.0, 12.0))
    m = compute_border_coverage_metrics(mask, route)
    assert m.mask_border_touch_count == 0
    assert m.route_border_point_count == 0
    assert m.route_points_within_2px_of_border == 0
    assert m.input_cropped is False
    assert m.min_route_border_clearance_px >= 5.0
